...
function handleGameGrid() {
    for (let i = 0; i < gameGrid.length; i++){
        gameGrid[i].draw(); // [i] represents object, zero, one, two...
    }
}
// Projectiles
...
function handleDefenders(){
    for (let i = 0; i < defender.length; i++){
        defender[i].draw();
        defender[i].update();
        if (enemyPositions.indexOf(defender[i].y) !== -1){
...
function handleGameStatus(){
    ...
    ctx.fillText('Resource: ' + numberOfResources, 20, 80);
    if (gameOver){
        ctx.fillStyle = 'black';
...
function animate() {
    ...
    frame++;
    //console.log(frame); as the game runs frame increases
